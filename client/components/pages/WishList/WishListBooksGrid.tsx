...
import { RemoveWishListBooks} from "@/network/endpoints/UsersApi";
...
const WishListBookGrid = ({favorites, clerkId}: { favorites: favoritesResponse[], clerkId: string | null }) => {
    ...
    const handleBookRemove = async (commentId: number) => {
        const response = await RemoveWishListBooks(clerkId, commentId)
        router.refresh();
