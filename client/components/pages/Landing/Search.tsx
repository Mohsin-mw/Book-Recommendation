...
const Search = () => {
  ...
  return (
    <>
      <div className="my-2 inline-block rounded-full border-primary-500  lg:bg-white py-2 lg:pl-4 pr-4 drop-shadow-2xl  relative z-50 w-full md:w-auto ">
        <div
          id="#searchSection"
        ...
        >
          <div className="relative w-full">
            ...
            />
            <div className="bg-primary inline-block p-2 rounded-full text-white absolute right-2 top-1/2 -translate-y-1/2 md:hidden">
              <BiSearch />
            </div>
          </div>
          <Button className="my-2 rounded-full px-8 py-6 text-white bg-[#E8BA96] lg:ml-4 hidden md:flex-row-start">
            Rechercher
            <BiSearch size={20} className="ml-2" />
          </Button>
        </div>
        {suggestions?.books ? (
          <div className="absolute w-full bg-white rounded-md shadow-light100 top-[110%] overflow-hidden ">
            <div className={"px-4 flex-column-start"}>
              {suggestions.books.map((book, i) => (
                ...
                >
                  {book.title}
                  <span className="text-quaternary text-sm opacity-0 group-hover:opacity-100 duration-200">
                    {" "}
                    ~ Auteur: {book.author}
                  </span>
                </Link>
            ...
            <div
              ...
              className={
                "py-2 px-4 cursor-pointer border-b-2 hover:border-b-primary duration-200 text-white bg-primary text-[14px] hover:text-[15px] w-full"
              }
            >
              Request Nouveau Livre
            </div>
          </div>
        ) : (
          ""
